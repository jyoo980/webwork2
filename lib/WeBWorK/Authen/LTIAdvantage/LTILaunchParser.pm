...
sub get_user_info {
	...
	$user{'deployment_id'} = $self->get_claim("deployment_id");
...
sub get_user_identifier {
	...
	my $deployment_id = $self->get_claim("deployment_id");
...
sub get_permissions {
	...
	foreach my $role (@roles) {
		if ($role eq 'http://purl.imsglobal.org/vocab/lis/v2/membership#Administrator') {
			$is_admin = 1;
		# TODO: remove || $role eq 'http://purl.imsglobal.org/vocab/lis/v2/institution/person#Instructor'
		} elsif ($role eq 'http://purl.imsglobal.org/vocab/lis/v2/membership#Instructor' || $role eq 'http://purl.imsglobal.org/vocab/lis/v2/institution/person#Instructor') {
			$is_instructor = 1;
		} elsif ($role eq 'http://purl.imsglobal.org/vocab/lis/v2/membership#ContentDeveloper') {
			$is_content_developer = 1;
		} elsif ($role eq 'http://purl.imsglobal.org/vocab/lis/v2/membership/Instructor#TeachingAssistant') {
			$is_ta = 1;
		} elsif ($role eq 'http://purl.imsglobal.org/vocab/lis/v2/membership#Learner') {
			$is_student = 1;
		}
	}
