...
sub pushUserGradesOnSubmit {
	...
	debug(Dumper($lti_assignment_and_grade_requests));
...
sub _generate_requests {
	...
	foreach my $lti_resource_link (@{$lti_resource_links}) {
		...
		my @deployment_lti_users = grep { $_->deployment_id() eq $deployment_id } @{$lti_users};

		my @grades_to_update = ();
		foreach my $lti_user (@deployment_lti_users) {
...
sub _performAssignmentAndGradeRequests {
	...
	$extralog->logLTIRequest("Begining LTI Assignment and Grade Service requests for Course: ".$ce->{courseName});
	debug("Begining LTI Assignment and Grade Service requests for Course: ".$ce->{courseName});
	...
	foreach my $lti_assignment_and_grade_request (@{$lti_assignment_and_grade_requests}) {
		...
		$extralog->logLTIRequest("Begining LTI Assignment and Grade Service request for Deployment: $deployment_id, Context: $context_id, Resource Link: $resource_link_id");
		debug("Begining LTI Assignment and Grade Service request for Deployment: $deployment_id, Context: $context_id, Resource Link: $resource_link_id");
		...
		if (!defined($lineitem_url) || $lineitem_url eq '') {
			...
			my $access_token = $lti_access_token_request->getAssignmentAndGradesLineItemAccessToken();
			unless ($access_token) {
				$self->{error} = "Assignment and Grades Service (LineItems POST) request failed, unable to get an access token.";
			...
			my $params = {
				scoreMaximum => 1.0,
				label => $label,
				ltiLinkId => $resource_link_id,
				tag => 'WebWork'
			};

			$extralog->logLTIRequest("Assignment and Grades Service (LineItems POST) request url: $lineitems_url, params: ".Dumper($params));
			...
			if ($res->is_success) {
				...
				$extralog->logLTIRequest("Assignment and Grades Service (LineItems POST): Added new Line Item: \n" . Dumper($data) . "\n");
				...
				$self->{error} = "Assignment and Grades Service (LineItems POST) request failed, unable to connect.";
				$extralog->logLTIRequest($self->{error});
				next;
			}
			...
			my $access_token = $lti_access_token_request->getAssignmentAndGradesLineItemAccessToken();
			unless ($access_token) {
				$self->{error} = "Assignment and Grades Service (LineItem GET) request failed, unable to get an access token.";
				$extralog->logLTIRequest($self->{error});
				next;
			}
			...
			$extralog->logLTIRequest("Assignment and Grades Service (LineItem GET) request url: $lineitem_url");
			...
			if ($res->is_success) {
				...
				$extralog->logLTIRequest("Assignment and Grades Service (LineItem GET): Get Line Item: \n" . Dumper($data) . "\n");
				debug("Assignment and Grades Service (LineItem GET): Get Line Item: \n" . Dumper($data) . "\n");

				$data->{label} = $label;

				if (defined($data->{'id'}) && $data->{'id'} ne $label) {
					my $ua = LWP::UserAgent->new();
					$ua->default_header( 'Accept' => 'application/vnd.ims.lis.v2.lineitem+json' );
					$ua->default_header( 'Authorization' => "Bearer $access_token");

					$extralog->logLTIRequest("Assignment and Grades Service (LineItem PUT) request url: $lineitem_url, params: ".Dumper($data));
					debug("Assignment and Grades Service (LineItem PUT) request url: $lineitem_url, params: ".Dumper($data));
					my $res = $ua->put($lineitem_url, $data);

					if ($res->is_success) {
						my $data = from_json($res->content);

						$extralog->logLTIRequest("Assignment and Grades Service (LineItem PUT): Successfuly updated label: \n" . Dumper($data) . "\n");
						debug("Assignment and Grades Service (LineItem PUT): Successfuly updated label: \n" . Dumper($data) . "\n");
					} else {
						debug($res->status_line);
						$self->{error} = "Assignment and Grades Service (LineItem PUT) request failed, unable to connect.";
						$extralog->logLTIRequest($self->{error});
						next;
					}
				}
				...
				debug($res->status_line);
				$self->{error} = "Assignment and Grades Service (LineItem GET) request failed, unable to connect.";
				$extralog->logLTIRequest($self->{error});
				next;
			}
		...
		if ($lti_resource_link->scope_result_readonly()) {
			...
			my $access_token = $lti_access_token_request->getAssignmentAndGradesResultReadonlyAccessToken();
			unless ($access_token) {
				$self->{error} = "Assignment and Grades Service (LineItem Result GET) request failed, unable to get an access token.";
				$extralog->logLTIRequest($self->{error});
				next;
			}
			...
			$extralog->logLTIRequest("Assignment and Grades Service (LineItem Result GET) request url: $lineitem_url/results");
			debug("Assignment and Grades Service (LineItem Result GET) request url: $lineitem_url/results");
			my $res = $ua->get($lineitem_url."/results");

			if ($res->is_success) {
				my $data = from_json($res->content);

				$extralog->logLTIRequest("Assignment and Grades Service (LineItem Result GET): Found these results: \n" . Dumper($data) . "\n");
				...
				# only keep grades_to_update that don't have a grade pushed yet or are different from what's currently stored
				my @grades_to_update = grep {
					!defined($lti_results->{$_->{lti_user_id}}) || $lti_results->{$_->{lti_user_id}} ne $_->{grade}
				} @grades_to_update;
			} else {
				debug($res->status_line);
				$self->{error} = "Assignment and Grades Service (LineItem Result GET) request failed, unable to connect.";
				$extralog->logLTIRequest($self->{error});
				next;
			}
		}

		# time to push new grade
		my $access_token = $lti_access_token_request->getAssignmentAndGradesScoreAccessToken();
		unless ($access_token) {
			$self->{error} = "Assignment and Grades Service (LineItem Score POST) request failed, unable to get an access token.";
			$extralog->logLTIRequest($self->{error});
			next;
		}
		...
		foreach my $grade_to_update (@grades_to_update) {
			...
			$extralog->logLTIRequest("Updating Grade for User: $user_id, LTI User: $lti_user_id, Set: $set_id, Grade: $grade, Deployment: $deployment_id, Context: $context_id, Resource Link: $resource_link_id");
			debug("Updating Grade for User: $user_id, LTI User: $lti_user_id, Set: $set_id, Grade: $grade, Deployment: $deployment_id, Context: $context_id, Resource Link: $resource_link_id");

			my $ua = LWP::UserAgent->new();
			$ua->default_header( 'Accept' => 'application/vnd.ims.lis.v1.score+json' );
			$ua->default_header( 'Authorization' => "Bearer $access_token");
			my $params = {
				userId => $lti_user_id,
				scoreGiven => $grade,
			...
			$extralog->logLTIRequest("Assignment and Grades Service (LineItem Score POST) request url: $lineitem_url/scores, params: ".Dumper($params));
			debug("Assignment and Grades Service (LineItem Score POST) request url: $lineitem_url/scores, params: ".Dumper($params));
			my $res = $ua->post($lineitem_url."/scores", $params);

			if ($res->is_success) {
				my $data = from_json($res->content);

				$extralog->logLTIRequest("Assignment and Grades Service (LineItem Score POST) request: Found these results: \n" . Dumper($data) . "\n");
				debug("Assignment and Grades Service (LineItem Score POST): Found these results: \n" . Dumper($data) . "\n");
			} else {
				debug($res->status_line);
				$self->{error} = "Assignment and Grades Service (LineItem Score POST) request failed, unable to connect.";
				$extralog->logLTIRequest($self->{error});
				next;
			}
		}
